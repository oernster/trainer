...
class WeatherConfig(BaseModel):
    ...
    refresh_interval_minutes: int = Field(
        default=30, ge=15, le=120, description="Weather refresh interval in minutes"
    )
    ...
    cache_duration_minutes: int = Field(
        default=30, ge=5, le=60, description="Weather data cache duration"
    )
    max_retries: int = Field(
        default=3, ge=1, le=10, description="Maximum API retry attempts"
    )
    timeout_seconds: int = Field(
        default=10, ge=5, le=30, description="API request timeout"
    )

    # Metadata
    api_provider: str = Field(
        default=__weather_api_provider__, description="Weather API provider name"
    )
    api_url: str = Field(
        default=__weather_api_url__, description="Weather API base URL"
    )
    config_version: str = Field(
        default=__weather_version__, description="Weather configuration version"
    )
...
class WeatherConfigValidator:
    ...
    def validate_config(cls, config: WeatherConfig) -> tuple[bool, list[str]]:
        ...
        if not cls.validate_coordinates(
            config.location_latitude, config.location_longitude
        ):
...
class WeatherConfigMigrator:
    ...
    def migrate_to_current_version(config_dict: dict) -> dict:
        ...
        logger.info(
            f"Migrating weather config from {config_version} to {current_version}"
        )
