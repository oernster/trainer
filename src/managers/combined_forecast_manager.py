...
class CombinedForecastManager(QObject):
    ...
    def _connect_manager_signals(self) -> None:
        ...
        if self._weather_manager:
            ...
            self._weather_manager.loading_state_changed.connect(
                self._on_weather_loading_changed
            )

        if self._astronomy_manager:
            self._astronomy_manager.astronomy_updated.connect(
                self._on_astronomy_updated
            )
            self._astronomy_manager.astronomy_error.connect(self._on_astronomy_error)
            self._astronomy_manager.loading_state_changed.connect(
                self._on_astronomy_loading_changed
            )
    ...
    def set_weather_manager(self, weather_manager: WeatherManager) -> None:
        ...
        if self._weather_manager:
            ...
            self._weather_manager.loading_state_changed.disconnect(
                self._on_weather_loading_changed
            )
    ...
    def set_astronomy_manager(self, astronomy_manager: AstronomyManager) -> None:
        ...
        if self._astronomy_manager:
            # Disconnect old signals
            self._astronomy_manager.astronomy_updated.disconnect(
                self._on_astronomy_updated
            )
            self._astronomy_manager.astronomy_error.disconnect(self._on_astronomy_error)
            self._astronomy_manager.loading_state_changed.disconnect(
                self._on_astronomy_loading_changed
            )
    ...
    async def get_combined_forecast(
        self, location: Optional[Location] = None, force_refresh: bool = False
        ...
        try:
            ...
            logger.info(
                f"Combined forecast updated: {combined_forecast.get_status_summary()}"
            )
    ...
    async def _fetch_astronomy_data(
        self, location: Optional[Location]
    ...
    def _on_astronomy_updated(self, astronomy_data: AstronomyForecastData) -> None:
        ...
        # Trigger combined forecast update
        asyncio.create_task(
            self._update_combined_forecast_from_astronomy(astronomy_data)
        )

    async def _update_combined_forecast_from_weather(
        self, weather_data: WeatherForecastData
    ) -> None:
    ...
    async def _update_combined_forecast_from_astronomy(
        self, astronomy_data: AstronomyForecastData
    ) -> None:
    ...
    def _update_combined_loading_state(self) -> None:
        ...
        if self._weather_manager:
            weather_loading = getattr(
                self._weather_manager, "is_loading", lambda: False
            )()

        if self._astronomy_manager:
            astronomy_loading = getattr(
                self._astronomy_manager, "is_loading", lambda: False
            )()
    ...
    def start_auto_refresh(self, interval_minutes: int = 30) -> None:
        ...
        logger.info(
            f"Combined forecast auto-refresh started (interval: {interval_minutes}m)"
        )
