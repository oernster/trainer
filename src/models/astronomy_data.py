...
class AstronomyData:
    ...
    def high_priority_events(self) -> List[AstronomyEvent]:
        ...
        return [
            ...
            if e.priority
            in [AstronomyEventPriority.HIGH, AstronomyEventPriority.CRITICAL]
        ]
    ...
    def get_events_by_priority(
        self, priority: AstronomyEventPriority
    ) -> List[AstronomyEvent]:
...
class AstronomyForecastData:
    ...
    def __post_init__(self):
        ...
        if len(self.daily_astronomy) > self.forecast_days:
            raise ValueError(
                f"Forecast cannot contain more than {self.forecast_days} days"
            )
    ...
    def get_astronomy_for_date(self, target_date: date) -> Optional[AstronomyData]:
        ...
        return next(
            (data for data in self.daily_astronomy if data.date == target_date), None
        )
...
class AstronomyIconProviderImpl:
    ...
    def __init__(self, strategy: AstronomyIconStrategy):
        ...
        logger.info(
            f"AstronomyIconProvider initialized with {strategy.get_strategy_name()} strategy"
        )
    ...
    def set_strategy(self, strategy: AstronomyIconStrategy) -> None:
        ...
        self._strategy = strategy
        logger.info(
            f"Astronomy icon strategy changed from {old_strategy} to {strategy.get_strategy_name()}"
        )
...
default_astronomy_icon_provider = AstronomyIconProviderImpl(
