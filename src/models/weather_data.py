...
class WeatherForecastData:
    ...
    def get_current_weather(self) -> Optional[WeatherData]:
        ...
        now = datetime.now()
        return min(
            self.hourly_forecast, key=lambda w: abs((w.timestamp - now).total_seconds())
        )
...
class WeatherIconProviderImpl:
    ...
    def __init__(self, strategy: WeatherIconStrategy):
        ...
        logger.info(
            f"WeatherIconProvider initialized with {strategy.get_strategy_name()} strategy"
        )
    ...
    def set_strategy(self, strategy: WeatherIconStrategy) -> None:
        ...
        self._strategy = strategy
        logger.info(
            f"Icon strategy changed from {old_strategy} to {strategy.get_strategy_name()}"
        )
...
class WeatherDataValidator:
    ...
    def validate_weather_data(cls, weather_data: WeatherData) -> bool:
        ...
        return (
            cls.validate_temperature(weather_data.temperature)
            and cls.validate_humidity(weather_data.humidity)
            and cls.validate_weather_code(weather_data.weather_code)
            and cls.validate_timestamp(weather_data.timestamp)
        )
