    ...
    def _parse_hourly_data(self, hourly_data: Dict) -> List[WeatherData]:
        ...
        for i, time_str in enumerate(times):
            try:
                ...
                if (i >= len(temperatures) or i >= len(humidities) or 
                    i >= len(weather_codes)):
                    continue
    ...
    def _parse_daily_data(self, daily_data: Dict) -> List[WeatherData]:
        ...
        for i, time_str in enumerate(times):
            try:
                ...
                # Skip if we don't have all required data
                if (i >= len(max_temps) or i >= len(min_temps) or 
                    i >= len(weather_codes)):
                    continue
...
class WeatherAPIManager:
    ...
    def __init__(
        ...
        logger.info(f"WeatherAPIManager initialized with {weather_source.get_source_name()}")
