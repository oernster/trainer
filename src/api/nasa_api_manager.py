    ...
    async def _ensure_session(self) -> aiohttp.ClientSession:
        ...
        if self._session is None or self._session.closed:
            self._session = aiohttp.ClientSession(
                ...
                headers={"User-Agent": f"Trainer/{__version__}"}
            )
...
class APODService(AstronomyService):
    ...
    async def _fetch_apod_batch(self, start_date: date, end_date: date) -> List[Dict[str, Any]]:
        ...
        if actual_start > actual_end:
            logger.info(f"No APOD data available for date range {start_date} to {end_date}")
            return []
        ...
        try:
            ...
            if response.status_code == 200:
                if isinstance(response.data, list):
                    ...
                    logger.warning(f"APOD API returned unexpected data type: {type(response.data)}")
                    return []
                ...
                logger.warning(f"APOD API returned 400 for date range {actual_start} to {actual_end} - likely no data available")
                return []
            else:
    ...
    def _parse_apod_response(self, data: Dict[str, Any], target_date: date) -> AstronomyEvent:
...
class ISSService(AstronomyService):
        ...
        try:
            ...
            if response.status_code == 200:
                if isinstance(response.data, dict):
                    ...
                    logger.warning(f"ISS API returned unexpected data type: {type(response.data)}")
                    return []
            elif response.status_code == 404:
                logger.warning("ISS API endpoint not found (404) - service may be temporarily unavailable")
                return []
            else:
            ...
            if "404" in str(e) or "text/html" in str(e):
                logger.warning("ISS API service appears to be unavailable - skipping ISS data")
                return []
            else:
    ...
    def _create_iss_event(self, pass_data: Dict[str, Any], location: Location) -> AstronomyEvent:
        ...
        visibility = "Visible pass" if duration > 300 else "Brief pass"  # 5+ minutes = good visibility
        ...
        return AstronomyEvent(
            ...
            image_url=None,
            priority=AstronomyEventPriority.HIGH if duration > 300 else AstronomyEventPriority.MEDIUM,
            metadata={
...
class NeoWsService(AstronomyService):
            ...
            for date_str, objects in neo_data.items():
                try:
                    ...
                    interesting_objects = [
                        ...
                        self._get_max_diameter(obj) > 100
                    ]
    ...
    def _get_max_diameter(self, obj: Dict[str, Any]) -> float:
        ...
        try:
            return float(obj.get('estimated_diameter', {}).get('meters', {}).get('estimated_diameter_max', 0))
        except (ValueError, TypeError):
            return 0.0
    
    async def _fetch_neo_data(self, start_date: date, end_date: date) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if response.status_code == 200:
            if isinstance(response.data, dict):
                ...
                logger.warning(f"NeoWs API returned unexpected data type: {type(response.data)}")
                return {}
        elif response.status_code == 403:
            raise AstronomyAuthenticationException("Invalid NASA API key")
        elif response.status_code == 429:
            ...
            raise AstronomyAPIException(f"NeoWs API returned status {response.status_code}")
    
    def _create_neo_event(self, objects: List[Dict[str, Any]], event_date: date) -> AstronomyEvent:
        """Create Near Earth Object event from API data."""
        object_count = len(objects)
        hazardous_count = sum(1 for obj in objects if obj.get('is_potentially_hazardous_asteroid', False))
        ...
        if hazardous_count > 0:
            ...
            title = f"{object_count} Near Earth Object{'s' if object_count != 1 else ''}"
...
class EPICService(AstronomyService):
    ...
    def _create_epic_event(self, image_data: Dict[str, Any], target_date: date) -> AstronomyEvent:
        ...
        image_url = f"https://api.nasa.gov/EPIC/archive/natural/{date_str}/png/{image_name}.png"
...
class NASAAstronomySource(AstronomyDataSource):
    ...
    def __init__(self, http_client: HTTPClient, config: AstronomyConfig):
        ...
        logger.info(f"NASAAstronomySource initialized with {len(self._services)} services")
    ...
    async def fetch_astronomy_data(self, location: Location, days: int) -> AstronomyForecastData:
        ...
        try:
            ...
            logger.info(f"Parsed astronomy data: {len(daily_astronomy)} days, "
    ...
    def _combine_service_results(
        ...
        while current_date <= end_date:
            ...
            astronomy_data = AstronomyData(
                ...
                moon_phase=self._calculate_moon_phase(current_date),  # Simplified calculation
                moon_illumination=self._calculate_moon_illumination(current_date)  # Simplified calculation
            )
...
class AstronomyAPIManager:
    ...
    def __init__(self, astronomy_source: AstronomyDataSource, config: AstronomyConfig):
        ...
        logger.info(f"AstronomyAPIManager initialized with {astronomy_source.get_source_name()}")
