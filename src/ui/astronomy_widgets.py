...
from PySide6.QtWidgets import (
    ...
    QScrollArea, QGridLayout, QSizePolicy, QSpacerItem, QToolTip
)
from PySide6.QtCore import (
    Qt, Signal, QPropertyAnimation, QEasingCurve, QTimer, QUrl
)
...
class AstronomyEventIcon(QLabel):
    ...
    def _setup_ui(self) -> None:
        ...
        self.setFixedSize(100, 100)  # Extra large size to fully accommodate big icons and hover borders
...
class DailyAstronomyPanel(QFrame):
    ...
    def _setup_ui(self) -> None:
        ...
        self._icons_layout.setContentsMargins(2, 4, 2, 4)  # Add vertical margins for icon spacing
        ...
        self._moon_label.setStyleSheet("background: transparent; border: none; font-size: 32px; font-family: 'Segoe UI Emoji', 'Apple Color Emoji', 'Noto Color Emoji';")
        layout.addWidget(self._moon_label)
        
        # Set size policy - extra large height for much larger icons with borders
        self.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        self.setFixedHeight(210)  # Extra large height for largest icon containers and borders
...
class AstronomyForecastPanel(QWidget):
    ...
    def update_forecast(self, forecast_data: AstronomyForecastData) -> None:
        ...
        logger.debug(f"Updated astronomy forecast panel with {len(forecast_data.daily_astronomy)} days")
...
class AstronomyEventDetails(QFrame):
    ...
    def _show_no_events(self, event_date: date) -> None:
        ...
        no_events_label.setStyleSheet("color: #888888; font-style: italic; margin-top: 16px;")
    ...
    def _create_moon_info_widget(self, astronomy_data: AstronomyData) -> QWidget:
        ...
        if astronomy_data.moon_illumination is not None:
            illumination_label = QLabel(f"Illumination: {astronomy_data.moon_illumination:.1%}")
    ...
    def _create_event_widget(self, event: AstronomyEvent) -> QWidget:
        ...
        if event.nasa_url:
            ...
            """)
            if event.nasa_url:
                link_button.clicked.connect(lambda url=event.nasa_url: self._open_nasa_link(url))
...
class AstronomyWidget(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self.setMaximumHeight(460)  # Increased to accommodate extra large daily panels (210px) plus button and spacing
