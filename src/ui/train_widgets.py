...
class TrainItemWidget(QFrame):
                    ...
                    if hasattr(self.train_data, 'route_segments') and self.train_data.route_segments:
                        ...
                        for j, segment in enumerate(self.train_data.route_segments):
                            ...
                            if connects_stations:
                                # Check for walking connections by multiple indicators
    ...
    def _is_major_interchange(self, station_name: str) -> bool:
        ...
        if hasattr(self.train_data, 'route_segments') and self.train_data.route_segments:
            # Use the new InterchangeDetectionService for intelligent detection
            ...
            for interchange in interchanges:
                if interchange.station_name == clean_name and interchange.is_user_journey_change:
                    logger.debug(f"Station {clean_name} is a valid user journey interchange")
                    return True
            
            logger.debug(f"Station {clean_name} is not a user journey interchange")
            return False
        else:
