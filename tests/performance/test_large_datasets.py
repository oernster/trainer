...
class TestPerformance:
    ...
    def test_train_data_creation_performance(self):
        ...
        assert creation_time < 1.0, f"Train creation took {creation_time:.2f}s, expected < 1.0s"
    ...
    def test_train_data_property_access_performance(self):
        ...
        assert access_time < 0.5, f"Property access took {access_time:.2f}s, expected < 0.5s"
    ...
    def test_train_data_display_dict_performance(self):
        ...
        assert conversion_time < 0.5, f"Display dict conversion took {conversion_time:.2f}s, expected < 0.5s"
    ...
    def test_train_sorting_performance(self):
        ...
        for i in range(len(sorted_trains) - 1):
            assert sorted_trains[i].departure_time <= sorted_trains[i + 1].departure_time
    ...
    def test_train_filtering_performance(self):
        ...
        fast_trains = [train for train in trains if train.service_type == ServiceType.FAST]
    ...
    def test_concurrent_train_operations(self):
        ...
        assert concurrent_time < 1.0, f"Concurrent operations took {concurrent_time:.2f}s, expected < 1.0s"
    ...
    def test_train_data_serialization_performance(self):
        ...
        assert serialization_time < 0.2, f"Serialization took {serialization_time:.2f}s, expected < 0.2s"
    ...
    def test_train_data_bulk_operations_scaling(self):
        ...
        assert scaling_factor < 6, f"Operations scale poorly: {scaling_factor:.1f}x slower for 4x data"
        
        # All operations should complete in reasonable time
        for i, (size, time_taken) in enumerate(zip(dataset_sizes, times)):
            max_expected_time = size / 1000.0  # 1 second per 1000 items
            assert time_taken < max_expected_time, f"Size {size} took {time_taken:.2f}s, expected < {max_expected_time:.2f}s"
...
class TestMemoryEfficiency:
    ...
    def test_train_data_memory_footprint(self):
        ...
        assert train_size < 1024, f"Train object size {train_size} bytes, expected < 1024 bytes"
    ...
    def test_large_dataset_memory_efficiency(self):
        ...
        assert memory_per_train < 2048, f"Memory per train: {memory_per_train:.0f} bytes, expected < 2048 bytes"
