...
class TestAioHttpClient:
    ...
    async def test_ensure_session_creates_new_session(self):
        ...
        with patch('src.api.weather_api_manager.aiohttp.ClientSession') as mock_session_class:
    ...
    def test_close_sync_with_running_loop(self):
        ...
        with patch('src.api.weather_api_manager.asyncio.get_event_loop') as mock_get_loop:
            mock_loop = Mock()
    ...
    def test_close_sync_no_event_loop(self):
        ...
        with patch('src.api.weather_api_manager.asyncio.get_event_loop') as mock_get_loop, \
             patch('src.api.weather_api_manager.asyncio.run') as mock_run:
    ...
    def test_close_sync_with_session_close_exception(self):
        ...
        with patch('src.api.weather_api_manager.asyncio.get_event_loop') as mock_get_loop:
            mock_loop = Mock()
            mock_loop.is_running.return_value = False
            mock_loop.run_until_complete.side_effect = Exception("Close operation failed")
            mock_get_loop.return_value = mock_loop
...
class TestOpenMeteoWeatherSource:
    ...
    async def test_fetch_weather_data_success(self, weather_source):
        ...
        api_data = {
            "hourly": {
                "time": ["2023-01-01T00:00:00", "2023-01-01T03:00:00", "2023-01-01T06:00:00"],
        ...
        with patch.object(weather_source, '_parse_hourly_data') as mock_hourly, \
             patch.object(weather_source, '_parse_daily_data') as mock_daily, \
             patch.object(weather_source, '_filter_to_3hourly') as mock_filter, \
             patch.object(weather_source._validator, 'validate_forecast_data', return_value=True):
    ...
    @pytest.mark.asyncio
    async def test_fetch_weather_data_weather_api_exception_passthrough(self, weather_source):
    ...
    def test_parse_api_response_validation_failure(self, weather_source):
        ...
        with patch.object(weather_source, '_parse_hourly_data') as mock_hourly, \
             patch.object(weather_source, '_parse_daily_data') as mock_daily, \
             patch.object(weather_source, '_filter_to_3hourly') as mock_filter, \
             patch.object(weather_source._validator, 'validate_forecast_data', return_value=False):
    ...
    def test_parse_hourly_data_success(self, weather_source):
        ...
        hourly_data = {
            "time": ["2023-01-01T00:00:00", "2023-01-01T03:00:00"],  # Remove Z to avoid timezone issues
            "temperature_2m": [10.0, 12.0],
            "relative_humidity_2m": [80, 75],
            "weather_code": [1, 2]
        }
        
        with patch.object(weather_source._validator, 'validate_weather_data', return_value=True):
    ...
    def test_parse_hourly_data_validation_failure(self, weather_source):
        ...
        with patch.object(weather_source._validator, 'validate_weather_data', return_value=False):
            result = weather_source._parse_hourly_data(hourly_data)
    ...
    def test_get_weather_description_known_codes(self, weather_source):
        ...
        assert weather_source._get_weather_description(99) == "Thunderstorm with heavy hail"
...
class TestWeatherAPIManager:
    ...
    async def test_get_weather_forecast_with_location(self, weather_manager):
        ...
        weather_manager._weather_source.fetch_weather_data.assert_called_once_with(location)
    ...
    async def test_get_weather_forecast_without_location(self, weather_manager):
        ...
        # Mock forecast data
        forecast_data = create_test_weather_forecast_data(Location("Test Location", 51.5, -0.1))
    ...
    async def test_get_weather_forecast_with_cache_valid(self, weather_manager):
        ...
        weather_manager._last_fetch_time = datetime.now() - timedelta(minutes=10)  # Recent
    ...
    async def test_get_weather_forecast_fetch_error_with_cache(self, weather_manager):
        ...
        weather_manager._weather_source.fetch_weather_data.side_effect = Exception("Fetch failed")
    ...
    def test_is_cache_valid_no_fetch_time(self, weather_manager):
        ...
        weather_manager._cached_data = create_test_weather_forecast_data(Location("Test", 51.5, -0.1))
...
class TestIntegrationAndEdgeCases:
    ...
    async def test_full_integration_flow(self):
        ...
        with patch.object(manager._weather_source, 'fetch_weather_data', return_value=test_forecast) as mock_fetch:
            # Test the full flow
