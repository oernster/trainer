...
class TestDailyForecastData:
    ...
    def test_daily_forecast_validation_weather_date_mismatch(self):
        ...
        with pytest.raises(ValueError, match="Weather data date must match forecast date"):
            DailyForecastData(
                date=test_date,
    ...
    def test_daily_forecast_validation_astronomy_date_mismatch(self):
        ...
        with pytest.raises(ValueError, match="Astronomy data date must match forecast date"):
            DailyForecastData(
                date=test_date,
...
class TestCombinedForecastData:
    ...
    def test_create_factory_method_no_data(self):
        ...
        try:
            ...
            assert "Combined forecast must contain at least one daily forecast" in str(e)
    ...
    def test_get_weather_for_date_not_found(self):
        ...
        result = CombinedForecastData._get_weather_for_date(weather_forecast, target_date)
    ...
    def test_determine_data_quality_excellent(self):
        ...
        quality = CombinedForecastData._determine_data_quality(weather_data, astronomy_data)
    ...
    def test_determine_data_quality_partial(self):
        ...
        quality = CombinedForecastData._determine_data_quality(None, AstronomyData(date=test_date, events=[]))
        assert quality == ForecastDataQuality.PARTIAL
    ...
    def test_determine_status_complete(self):
        ...
        weather_forecast = WeatherForecastData(location=location, daily_forecast=[weather_data], hourly_forecast=[])
        ...
        astronomy_forecast = AstronomyForecastData(location=astronomy_location, daily_astronomy=[astronomy_data])
        
        status = CombinedForecastData._determine_status(weather_forecast, astronomy_forecast, [])
    ...
    def test_has_high_priority_astronomy_property(self):
        ...
        daily_forecast_high = DailyForecastData(date=today, astronomy_data=astronomy_data_high)
        
        forecast_with_high = CombinedForecastData(
            location=location,
            daily_forecasts=[daily_forecast_high]
        )
        ...
        daily_forecast_low = DailyForecastData(date=today, astronomy_data=astronomy_data_low)
        
        forecast_without_high = CombinedForecastData(
            location=location,
            daily_forecasts=[daily_forecast_low]
        )
    ...
    def test_get_forecasts_with_astronomy(self):
        ...
        forecast_with_astronomy = DailyForecastData(date=today, astronomy_data=astronomy_data)
        ...
        forecast_without_astronomy = DailyForecastData(date=tomorrow, weather_data=weather_data)
    ...
    def test_get_forecasts_with_weather(self):
        ...
        forecast_without_weather = DailyForecastData(date=tomorrow, astronomy_data=astronomy_data)
    ...
    def test_get_high_priority_astronomy_days(self):
        ...
        forecast_high = DailyForecastData(date=today, astronomy_data=astronomy_data_high)
        forecast_low = DailyForecastData(date=tomorrow, astronomy_data=astronomy_data_low)
        
        combined_forecast = CombinedForecastData(
            location=location,
            daily_forecasts=[forecast_high, forecast_low]
        )
    ...
    def test_get_precipitation_days(self):
        ...
        combined_forecast = CombinedForecastData(
            location=location,
            daily_forecasts=[forecast_rain, forecast_clear]
        )
    ...
    def test_get_error_summary_with_errors(self):
        ...
        assert combined_forecast.get_error_summary() == "Weather API error; Astronomy API timeout"
...
class TestCombinedForecastValidator:
    ...
    def test_validate_daily_forecast_exception(self):
        ...
        mock_forecast = MagicMock()
        mock_forecast.has_weather_data = True
        mock_forecast.weather_data.timestamp.date.side_effect = AttributeError("Test error")
    ...
    def test_validate_location_consistency_valid(self):
        ...
        assert CombinedForecastValidator.validate_location_consistency(combined_forecast)
    ...
    def test_validate_location_consistency_weather_mismatch(self):
        ...
        assert not CombinedForecastValidator.validate_location_consistency(combined_forecast)
    ...
    def test_validate_combined_forecast_invalid_daily(self):
        ...
        with patch.object(CombinedForecastValidator, 'validate_location_consistency', return_value=True):
            assert not CombinedForecastValidator.validate_combined_forecast(mock_forecast)
    ...
    def test_validate_combined_forecast_unordered_dates(self):
        ...
        with patch.object(CombinedForecastValidator, 'validate_location_consistency', return_value=True):
            with patch.object(CombinedForecastValidator, 'validate_daily_forecast', return_value=True):
                assert not CombinedForecastValidator.validate_combined_forecast(mock_forecast)
    ...
    def test_additional_coverage_scenarios(self):
        ...
        with pytest.raises(ValueError, match="Daily forecast must contain either weather or astronomy data"):
        ...
        assert daily_forecast_with_data.moon_phase_icon == "🌑"  # Default from astronomy_data
        
        # Test get_astronomy_events_by_type with data
        events = daily_forecast_with_data.get_astronomy_events_by_type(AstronomyEventType.APOD)
        assert len(events) == 1
    ...
    def test_combined_forecast_validation_errors(self):
        ...
        with pytest.raises(ValueError, match="Daily forecasts must be in chronological order"):
            CombinedForecastData(
                location=location,
        ...
        with pytest.raises(ValueError, match="Daily forecasts cannot contain duplicate dates"):
            CombinedForecastData(
                location=location,
        ...
        with pytest.raises(ValueError, match="Combined forecast cannot exceed 14 days"):
            CombinedForecastData(
                location=location,
                daily_forecasts=long_forecasts
            )
    ...
    def test_validator_exception_handling(self):
        ...
        invalid_forecast = None  # This will cause AttributeError when accessing attributes
...
class TestFactoryFunctions:
    ...
    def test_create_complete_forecast(self):
        ...
        result = create_complete_forecast(location, weather_forecast, astronomy_forecast)
