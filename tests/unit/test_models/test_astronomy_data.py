...
class TestAstronomyData:
    ...
    def test_astronomy_data_validation_moon_illumination_invalid(self):
        ...
        with pytest.raises(
            ValueError, match="Moon illumination must be between 0.0 and 1.0"
        ):
            AstronomyData(date=date.today(), moon_illumination=1.5)

    def test_astronomy_data_validation_sunrise_wrong_date(self):
        """Test validation with sunrise on wrong date."""
        test_date = date.today()
        wrong_date_sunrise = datetime.combine(
            test_date + timedelta(days=1), datetime.min.time()
        )
        sunset = datetime.combine(test_date, datetime.min.time().replace(hour=18))

        with pytest.raises(ValueError, match="Sunrise time must be on the same date"):
            AstronomyData(
                date=test_date, sunrise_time=wrong_date_sunrise, sunset_time=sunset
            )
    ...
    def test_astronomy_data_validation_sunset_wrong_date(self):
        ...
        test_date = date.today()
        sunrise = datetime.combine(test_date, datetime.min.time().replace(hour=6))
        wrong_date_sunset = datetime.combine(
            test_date + timedelta(days=1), datetime.min.time()
        )

        with pytest.raises(ValueError, match="Sunset time must be on the same date"):
            AstronomyData(
                date=test_date, sunrise_time=sunrise, sunset_time=wrong_date_sunset
            )
    ...
    def test_astronomy_data_validation_sunrise_after_sunset(self):
        ...
        with pytest.raises(ValueError, match="Sunrise must be before sunset"):
            AstronomyData(date=test_date, sunrise_time=sunrise, sunset_time=sunset)
    ...
    def test_astronomy_data_validation_primary_event_not_in_list(self):
        ...
        with pytest.raises(
            ValueError, match="Primary event must be in the events list"
        ):
            AstronomyData(date=test_date, events=[event1], primary_event=event2)
    ...
    def test_astronomy_data_validation_event_wrong_date(self):
        ...
        with pytest.raises(ValueError, match="Event Wrong Date Event is not for date"):
            AstronomyData(date=test_date, events=[event])
    ...
    def test_high_priority_events(self):
        ...
        astronomy_data = AstronomyData(
            date=test_date, events=[high_event, critical_event, low_event]
        )
    ...
    def test_moon_phase_icon(self):
        ...
        for moon_phase, expected_icon in icons.items():
            astronomy_data = AstronomyData(date=date.today(), moon_phase=moon_phase)
    ...
    def test_daylight_duration(self):
        ...
        astronomy_data = AstronomyData(
            date=test_date, sunrise_time=sunrise, sunset_time=sunset
        )
    ...
    def test_get_events_by_type(self):
        ...
        astronomy_data = AstronomyData(date=test_date, events=[apod_event, iss_event])
    ...
    def test_get_events_by_priority(self):
        ...
        astronomy_data = AstronomyData(date=test_date, events=[high_event, low_event])
    ...
    def test_get_ongoing_events(self, mock_datetime):
        ...
        astronomy_data = AstronomyData(
            date=test_date, events=[ongoing_event, future_event]
        )
    ...
    def test_get_future_events(self, mock_datetime):
        ...
        astronomy_data = AstronomyData(
            date=test_date, events=[past_event, future_event]
        )
    ...
    def test_get_sorted_events(self):
        ...
        event1 = AstronomyEvent(
            ...
            start_time=datetime.combine(
                test_date, datetime.min.time().replace(hour=14)
            ),
            priority=AstronomyEventPriority.HIGH,
        )
        ...
        astronomy_data = AstronomyData(date=test_date, events=[event1, event2])
...
class TestAstronomyForecastData:
    ...
    def test_astronomy_forecast_validation_empty_daily_data(self):
        ...
        with pytest.raises(
            ValueError, match="Forecast must contain at least one day of astronomy data"
        ):
            AstronomyForecastData(location=location, daily_astronomy=[])
    ...
    def test_astronomy_forecast_validation_too_many_days(self):
        ...
        daily_data = [
            AstronomyData(date=date.today() + timedelta(days=i)) for i in range(8)
        ]

        with pytest.raises(
            ValueError, match="Forecast cannot contain more than 7 days"
        ):
            AstronomyForecastData(
                location=location, daily_astronomy=daily_data, forecast_days=7
            )
    ...
    def test_astronomy_forecast_validation_unordered_dates(self):
        ...
        with pytest.raises(
            ValueError, match="Daily astronomy data must be in chronological order"
        ):
            AstronomyForecastData(location=location, daily_astronomy=daily_data)
    ...
    def test_astronomy_forecast_validation_duplicate_dates(self):
        ...
        today = date.today()
        daily_data = [AstronomyData(date=today), AstronomyData(date=today)]  # Duplicate

        with pytest.raises(
            ValueError, match="Daily astronomy data cannot contain duplicate dates"
        ):
            AstronomyForecastData(location=location, daily_astronomy=daily_data)
    ...
    def test_total_events(self):
        ...
        forecast = AstronomyForecastData(location=location, daily_astronomy=daily_data)
    ...
    def test_has_high_priority_events(self):
        ...
        high_priority_forecast = AstronomyForecastData(
            location=location, daily_astronomy=high_priority_data
        )
        ...
        low_priority_forecast = AstronomyForecastData(
            location=location, daily_astronomy=low_priority_data
        )
    ...
    def test_forecast_date_properties(self):
        ...
        daily_data = [AstronomyData(date=today), AstronomyData(date=tomorrow)]
    ...
    def test_get_today_astronomy(self, mock_date):
        ...
        forecast = AstronomyForecastData(
            location=location, daily_astronomy=[today_data]
        )
    ...
    def test_get_tomorrow_astronomy(self, mock_date):
        ...
        forecast = AstronomyForecastData(
            location=location, daily_astronomy=[tomorrow_data]
        )
...
class TestAstronomyDataValidator:
    ...
    def test_validate_astronomy_forecast(self):
        ...
        forecast = AstronomyForecastData(
            location=location, daily_astronomy=[astronomy_data]
        )
...
class TestEdgeCasesForFullCoverage:
    ...
    def test_validator_edge_cases_for_full_coverage(self):
        ...
        with patch.object(
            AstronomyDataValidator, "validate_astronomy_event", return_value=False
        ):
        ...
        with patch.object(
            AstronomyDataValidator, "validate_moon_phase", return_value=False
        ):
        ...
        with patch.object(
            AstronomyDataValidator, "validate_astronomy_data", return_value=False
        ):
            assert not AstronomyDataValidator.validate_astronomy_forecast(mock_forecast)
        ...
        mock_forecast2.daily_astronomy = [
            MagicMock() for _ in range(8)
        ]  # Too many days
        mock_forecast2.forecast_days = 7

        with patch.object(
            AstronomyDataValidator, "validate_location", return_value=True
        ):
            with patch.object(
                AstronomyDataValidator, "validate_astronomy_data", return_value=True
            ):
                assert not AstronomyDataValidator.validate_astronomy_forecast(
                    mock_forecast2
                )
