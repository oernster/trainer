...
try:
    from PySide6.QtWidgets import (
        QApplication, QMessageBox, QLineEdit, QPushButton, QComboBox, 
        QCheckBox, QTabWidget, QProgressDialog
    )
...
if HAS_QT:
    from src.ui.settings_dialog import SettingsDialog, HorizontalSpinWidget, APITestThread
    from src.managers.config_manager import (
        ConfigManager, ConfigurationError, ConfigData, APIConfig, 
        StationConfig, RefreshConfig, DisplayConfig
    )
    from src.managers.theme_manager import ThemeManager
    from src.api.api_manager import APIException, NetworkException, AuthenticationException
...
class TestHorizontalSpinWidget:
    ...
    def test_init_custom_values(self, qapp):
        ...
        widget = HorizontalSpinWidget(
            minimum=10, maximum=50, initial_value=25, 
            step=5, suffix=" items", theme_manager=theme_manager
        )
...
class TestSettingsDialog:
    ...
    def test_reset_to_defaults_confirmed(self, qapp, config_manager):
        ...
        with patch('src.ui.settings_dialog.QMessageBox') as mock_msgbox:
            ...
            with patch.object(dialog.config_manager, 'create_default_config') as mock_create:
    ...
    def test_reset_to_defaults_error(self, qapp, config_manager):
        ...
        with patch('src.ui.settings_dialog.QMessageBox') as mock_msgbox:
            ...
            with patch.object(dialog.config_manager, 'create_default_config', 
    ...
    def test_save_settings_configuration_error(self, qapp, config_manager):
        ...
        with patch.object(dialog.config_manager, 'save_config', 
...
@pytest.mark.skipif(not HAS_QT, reason="PySide6 not available")
@pytest.mark.filterwarnings("ignore:coroutine 'APITestThread._test_api' was never awaited:RuntimeWarning")
...
class TestAPITestThread:
    ...
    def test_run_success_with_trains(self, mock_api_manager_class, mock_test_api, test_config):
        # Configure the async mock to return a resolved future
        mock_test_api.return_value = (True, "Test successful")
        ...
        with patch.object(thread, '_test_api', side_effect=mock_test_api):
            ...
            signal_received = []
            thread.test_completed.connect(lambda success, msg: signal_received.append((success, msg)))
    ...
    def test_run_success_no_trains(self, mock_api_manager_class, mock_test_api, test_config):
        # Configure the async mock to return a resolved future
        mock_test_api.return_value = (True, "Test successful")
    ...
    def test_run_authentication_error(self, mock_api_manager_class, mock_test_api, test_config):
        # Configure the async mock to return a resolved future
        mock_test_api.return_value = (True, "Test successful")
    ...
    def test_run_network_error(self, mock_api_manager_class, mock_test_api, test_config):
        # Configure the async mock to return a resolved future
        mock_test_api.return_value = (True, "Test successful")
    ...
    def test_run_unexpected_error(self, mock_api_manager_class, mock_test_api, test_config):
    ...
    def test_run_exception_in_run_method(self, mock_api_manager_class, mock_test_api, test_config):
        # Configure the async mock to return a resolved future
        mock_test_api.return_value = (True, "Test successful")
        ...
        with patch('src.ui.settings_dialog.asyncio.new_event_loop', side_effect=Exception("Loop creation failed")):
            # Capture the signal emission
            signal_received = []
            thread.test_completed.connect(lambda success, msg: signal_received.append((success, msg)))
    ...
    def test_run_loop_cleanup(self, mock_api_manager_class, mock_test_api, test_config):
        ...
        with patch('src.ui.settings_dialog.asyncio.new_event_loop', return_value=mock_loop):
            with patch('src.ui.settings_dialog.asyncio.set_event_loop'):
                with patch.object(mock_loop, 'run_until_complete', side_effect=Exception("Test exception")):
                    # Capture the signal emission
                    signal_received = []
                    thread.test_completed.connect(lambda success, msg: signal_received.append((success, msg)))
    ...
    def test_run_loop_cleanup_when_none(self, mock_api_manager_class, mock_test_api, test_config):
        # Configure the async mock to return a resolved future
        mock_test_api.return_value = (True, "Test successful")
...
class TestSettingsDialogIntegration:
    ...
    def test_full_workflow_save_settings(self, mock_thread_class, mock_api_manager_class, mock_test_api, qapp, config_manager):
        # Configure the async mock to return a resolved future
        mock_test_api.return_value = (True, "Test successful")
    ...
    def test_horizontal_spin_widget_signal_integration(self, mock_thread_class, mock_api_manager_class, mock_test_api, qapp, config_manager):
        # Configure the async mock to return a resolved future
        mock_test_api.return_value = (True, "Test successful")
    ...
    def test_theme_manager_integration(self, mock_thread_class, mock_api_manager_class, mock_test_api, qapp, config_manager):
        # Configure the async mock to return a resolved future
        mock_test_api.return_value = (True, "Test successful")
