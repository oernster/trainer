...
class TestTrainListWidget:
    ...
    def test_scroll_to_bottom(self, qapp, large_train_dataset):
        ...
        assert widget.verticalScrollBar().value() == widget.verticalScrollBar().maximum()
...
class TestWidgetIntegration:
    ...
    def test_logging_integration(self, mock_logger, qapp, sample_train_data):
        ...
        mock_logger.info.assert_called_with("TrainListWidget initialized with max_trains=25")
        
        # Update trains and verify logging
        train_list.update_trains(sample_train_data)
        mock_logger.info.assert_called_with(f"Updated train list with {len(sample_train_data)} trains")