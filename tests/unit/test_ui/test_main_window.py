...
if HAS_QT:
    ...
    from src.managers.config_manager import ConfigManager, ConfigurationError, ConfigData, APIConfig, StationConfig, RefreshConfig, DisplayConfig
...
class TestMainWindow:
    ...
    def test_init_with_configuration_error(self, qapp):
        ...
        with patch('src.ui.main_window.ConfigManager') as mock_config_manager_class:
            ...
            with patch.object(MainWindow, 'show_error_message') as mock_show_error:
                ...
                mock_show_error.assert_called_once_with("Configuration Error", "Test error")
    ...
    def test_setup_ui_components(self, qapp, config_manager):
        ...
        assert "Showing trains for next 16 hours" in cast(QLabel, window.time_window_label).text()
    ...
    def test_on_theme_changed(self, qapp, config_manager):
        ...
        with patch.object(window, 'theme_changed') as mock_signal:
            ...
            assert cast(QPushButton, window.theme_button).text() == window.theme_manager.get_theme_icon()
    ...
    def test_update_train_display(self, qapp, config_manager, sample_train_data):
        ...
        assert f"{len(sample_train_data)} trains" in cast(QLabel, window.train_count_label).text()
        
        # Check train list widget is updated
        assert cast(TrainListWidget, window.train_list_widget).get_train_count() == len(sample_train_data)
    ...
    def test_update_last_update_time(self, qapp, config_manager):
        ...
        assert f"Last Updated: {timestamp}" in cast(QLabel, window.last_update_label).text()
    ...
    def test_update_connection_status_connected(self, qapp, config_manager):
        ...
        assert "#4caf50" in cast(QLabel, window.connection_status).styleSheet()  # Green color
    ...
    def test_update_connection_status_disconnected(self, qapp, config_manager):
        ...
        assert "Disconnected (Network error)" in cast(QLabel, window.connection_status).text()
        assert "#f44336" in cast(QLabel, window.connection_status).styleSheet()  # Red color
    ...
    def test_update_auto_refresh_status_enabled(self, qapp, config_manager):
        ...
        assert "Click to disable" in cast(QPushButton, window.auto_refresh_button).toolTip()
    ...
    def test_update_auto_refresh_status_disabled(self, qapp, config_manager):
        ...
        assert "Auto-refresh: OFF" in cast(QLabel, window.auto_refresh_status).text()
        assert "▶️ Auto-refresh" in cast(QPushButton, window.auto_refresh_button).text()
        assert "Click to enable" in cast(QPushButton, window.auto_refresh_button).toolTip()
    ...
    def test_show_settings_dialog_exception(self, qapp, config_manager):
        ...
        with patch('src.ui.main_window.SettingsDialog', side_effect=Exception("Dialog error")):
            with patch.object(window, 'show_error_message') as mock_show_error:
                window.show_settings_dialog()
                
                mock_show_error.assert_called_once_with(
                    "Settings Error", 
                    "Failed to open settings: Dialog error"
                )
    ...
    def test_on_settings_saved_configuration_error(self, qapp, config_manager):
        ...
        with patch.object(window.config_manager, 'load_config', side_effect=ConfigurationError("Load error")):
            with patch.object(window, 'show_error_message') as mock_show_error:
                window.on_settings_saved()
                
                mock_show_error.assert_called_once_with(
                    "Configuration Error",
                    "Failed to reload settings: Load error"
                )
    ...
    def test_train_list_widget_integration(self, qapp, config_manager, sample_train_data):
    ...
    def test_config_none_handling(self, qapp):
        ...
        with patch('src.ui.main_window.ConfigManager') as mock_config_manager_class:
            mock_config_manager = Mock()
            mock_config_manager.install_default_config_to_appdata.return_value = None
            mock_config_manager.load_config.side_effect = ConfigurationError("Config error")
            mock_config_manager_class.return_value = mock_config_manager
    ...
    def test_theme_toggle_with_config_save_error(self, qapp, config_manager):
        ...
        with patch.object(window.config_manager, 'save_config', side_effect=ConfigurationError("Save failed")):
            initial_theme = window.theme_manager.current_theme
            window.toggle_theme()
    ...
    def test_update_connection_status_disconnected_no_message(self, qapp, config_manager):
        ...
        assert "Disconnected" in cast(QLabel, window.connection_status).text()
        assert "(" not in cast(QLabel, window.connection_status).text()  # No message in parentheses
    ...
    def test_setup_application_icon_png_files(self, qapp, config_manager):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def mock_path_constructor(path_str):
                mock_path_obj = Mock()
                if path_str.startswith("assets/train_icon_") and path_str.endswith(".png"):
