...
try:
    ...
    from PySide6.QtCore import Qt, QPropertyAnimation, QEasingCurve, QUrl, Signal, QPoint
...
def sample_astronomy_data(sample_astronomy_event, sample_high_priority_event):
    ...
    return AstronomyData(
        ...
        sunrise_time=datetime.combine(date.today(), datetime.min.time().replace(hour=6)),
        sunset_time=datetime.combine(date.today(), datetime.min.time().replace(hour=20))
    )
...
def sample_astronomy_forecast(sample_location, sample_astronomy_data):
    ...
    for i in range(7):
        ...
        if i == 0:
            ...
            daily_data.append(AstronomyData(
                date=day_date,
                events=[],
...
def sample_astronomy_config():
    ...
    return AstronomyConfig(
        ...
        display=AstronomyDisplayConfig(
...
class TestDailyAstronomyPanel:
    ...
    def test_update_data_no_events(self, qapp, sample_astronomy_data_no_events):
        ...
        assert panel._moon_label.text() == sample_astronomy_data_no_events.moon_phase_icon
    ...
    def test_update_forecast(self, qapp, sample_astronomy_forecast):
        ...
        for i, daily_panel in enumerate(panel._daily_panels):
            if i < len(sample_astronomy_forecast.daily_astronomy):
                assert daily_panel._astronomy_data == sample_astronomy_forecast.daily_astronomy[i]
    ...
    def test_update_data_no_events(self, qapp, sample_astronomy_data_no_events):
        ...
        for i in range(details._layout.count()):
            ...
            if item and item.widget():
                widget = item.widget()
                if isinstance(widget, QLabel) and "No astronomy events" in widget.text():
...
class TestAstronomyExpandablePanel:
    ...
    def test_toggle_expansion_expand(self, qapp):
        ...
        # Connect signal to capture emissions
        signal_emitted = []
        panel.expansion_changed.connect(lambda expanded: signal_emitted.append(expanded))
    ...
    def test_update_details_fallback_to_first_day(self, qapp, sample_astronomy_forecast):
        ...
        from src.models.astronomy_data import AstronomyForecastData, AstronomyData, MoonPhase
        ...
        for i in range(3):
            ...
            daily_data = AstronomyData(
                ...
                moon_phase=MoonPhase.FULL_MOON,
                moon_illumination=0.98,
                sunrise_time=datetime.combine(current_date, datetime.min.time().replace(hour=6)),
                sunset_time=datetime.combine(current_date, datetime.min.time().replace(hour=20))
            )
...
class TestAstronomyWidget:
    ...
    def test_connect_signals(self, qapp):
        ...
        # Connect signal to capture emissions
        signal_emitted = []
        widget.astronomy_event_clicked.connect(lambda event: signal_emitted.append(event))
    ...
    def test_on_event_icon_clicked_with_nasa_url(self, qapp):
        ...
        nasa_signal_emitted = []
        widget.astronomy_event_clicked.connect(lambda event: astronomy_signal_emitted.append(event))
    ...
    def test_open_nasa_astronomy_page(self, qapp):
        ...
        with patch.object(widget, '_open_nasa_link') as mock_open_link:
            ...
            mock_open_link.assert_called_once_with("https://science.nasa.gov/astrophysics/")
    ...
    def test_update_config(self, qapp, sample_astronomy_config):
        ...
        assert widget.isVisible()  # Should be visible when enabled and show_in_forecast is True
    ...
    def test_update_config_disabled(self, qapp):
        ...
        config = AstronomyConfig(
            enabled=False,
            display=AstronomyDisplayConfig(show_in_forecast=True)
        )
...
class TestIntegration:
    ...
    def test_full_widget_integration(self, qapp, sample_astronomy_forecast, sample_astronomy_config):
    ...
    def test_event_icon_signal_chain(self, qapp, sample_astronomy_forecast):
        ...
        final_signals = []
        forecast_panel.event_icon_clicked.connect(lambda event: final_signals.append(event))
