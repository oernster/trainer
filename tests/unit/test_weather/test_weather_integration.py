...
class TestWeatherDataModels:
    ...
    def test_weather_data_validation(self):
        ...
        valid_weather = WeatherData(
            timestamp=datetime.now(), temperature=15.0, humidity=50, weather_code=0
        )
    ...
    def test_weather_forecast_data(self):
        ...
        now = datetime.now()
        hourly_data = [
            WeatherData(now + timedelta(hours=i), 20.0 + i, 50, 0) for i in range(8)
        ]
        daily_data = [
            WeatherData(now + timedelta(days=i), 18.0 + i, 55, 1) for i in range(7)
        ]

        forecast = WeatherForecastData(
            location=location, hourly_forecast=hourly_data, daily_forecast=daily_data
        )
...
class TestWeatherAPIIntegration:
    ...
    async def test_weather_api_manager_caching(self, weather_config):
        ...
        test_forecast = WeatherForecastData(
            location=location, hourly_forecast=[test_weather], daily_forecast=[]
        )
...
class TestWeatherIntegrationEnd2End:
    ...
    async def test_complete_weather_flow(self):
        ...
        with patch.object(manager._api_manager, "get_weather_forecast") as mock_fetch:
            ...
            test_forecast = WeatherForecastData(
                location=location,
                hourly_forecast=[WeatherData(datetime.now(), 20.0, 50, 0)],
                daily_forecast=[WeatherData(datetime.now(), 18.0, 55, 1)],
            )
