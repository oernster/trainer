...
class TestAstronomyManager:
    ...
    def mock_api_manager(self):
        ...
        api_manager.get_cache_info = Mock(return_value={
    ...
    def test_init_enabled_with_valid_api_key(self, mock_config):
        ...
        with patch('src.managers.astronomy_manager.AstronomyAPIFactory.create_manager_from_config') as mock_factory:
    ...
    @pytest.mark.asyncio
    async def test_refresh_astronomy_skip_refresh(self, mock_config, sample_forecast_data):
        """Test refresh skipped when recent data is available."""
        with patch('src.managers.astronomy_manager.AstronomyAPIFactory.create_manager_from_config'):
            manager = AstronomyManager(mock_config)
    ...
    @pytest.mark.asyncio
    async def test_refresh_astronomy_success(self, mock_config, mock_api_manager, sample_forecast_data):
        ...
        with patch('src.managers.astronomy_manager.AstronomyAPIFactory.create_manager_from_config') as mock_factory:
            ...
            with patch.object(manager._validator, 'validate_astronomy_forecast', return_value=True):
                result = await manager.refresh_astronomy()
    ...
    @pytest.mark.asyncio
    async def test_refresh_astronomy_validation_failure(self, mock_config, mock_api_manager, sample_forecast_data):
        ...
        with patch('src.managers.astronomy_manager.AstronomyAPIFactory.create_manager_from_config') as mock_factory:
            ...
            with patch.object(manager._validator, 'validate_astronomy_forecast', return_value=False):
                result = await manager.refresh_astronomy()
            
            assert result is None

    @pytest.mark.asyncio
    async def test_refresh_astronomy_authentication_exception(self, mock_config, mock_api_manager):
        """Test refresh with authentication exception."""
        mock_api_manager.get_astronomy_forecast.side_effect = AstronomyAuthenticationException("Auth failed")
    ...
    @pytest.mark.asyncio
    async def test_refresh_astronomy_rate_limit_exception(self, mock_config, mock_api_manager):
        """Test refresh with rate limit exception."""
        mock_api_manager.get_astronomy_forecast.side_effect = AstronomyRateLimitException("Rate limit exceeded")
    ...
    @pytest.mark.asyncio
    async def test_refresh_astronomy_network_exception(self, mock_config, mock_api_manager):
        """Test refresh with network exception."""
        mock_api_manager.get_astronomy_forecast.side_effect = AstronomyNetworkException("Network error")
    ...
    async def test_refresh_astronomy_api_exception(self, mock_config, mock_api_manager):
        ...
        mock_api_manager.get_astronomy_forecast.side_effect = AstronomyAPIException("API error")
    ...
    @pytest.mark.asyncio
    async def test_refresh_astronomy_generic_exception(self, mock_config, mock_api_manager):
    ...
    def test_is_data_stale_with_forecast(self, mock_config, sample_forecast_data):
        ...
        with patch('src.managers.astronomy_manager.AstronomyAPIFactory.create_manager_from_config'):
            ...
            with patch.object(type(sample_forecast_data), 'is_stale', new_callable=lambda: property(lambda self: False)):
    ...
    def test_clear_cache_with_api_manager(self, mock_config, mock_api_manager, sample_forecast_data):
    ...
    def test_update_config_api_key_change(self, mock_create_task, mock_config, mock_api_manager):
    ...
    def test_get_status_summary_stale_data(self, mock_config, mock_api_manager, sample_forecast_data):
    ...
    def test_get_status_summary_current_data(self, mock_config, mock_api_manager, sample_forecast_data):
    ...
    @patch('asyncio.get_event_loop')
    @patch('asyncio.run')
    def test_shutdown_sync_shutdown_success(self, mock_run, mock_get_loop, mock_config, mock_api_manager):
    ...
    @patch('asyncio.get_event_loop')
    @patch('asyncio.create_task')
    def test_shutdown_sync_shutdown_failure_async_fallback(self, mock_create_task, mock_get_loop, mock_config, mock_api_manager):
    ...
    @patch('asyncio.get_event_loop')
    @patch('asyncio.run')
    def test_shutdown_sync_shutdown_failure_async_fallback_not_running(self, mock_config, mock_api_manager):
...
class TestAstronomyManagerIntegration:
    ...
    def test_signal_emissions(self, real_config):
        ...
        with patch('src.managers.astronomy_manager.AstronomyAPIFactory.create_manager_from_config'):
            ...
            manager.astronomy_updated.connect(lambda data: astronomy_updated_calls.append(data))
            manager.astronomy_error.connect(lambda error: astronomy_error_calls.append(error))
            manager.loading_state_changed.connect(lambda state: loading_state_calls.append(state))
            manager.cache_status_changed.connect(lambda status: cache_status_calls.append(status))
