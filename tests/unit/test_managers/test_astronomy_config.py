...
class TestAstronomyDisplayConfig:
    ...
    def test_post_init_validation_max_events_too_low(self):
        ...
        with pytest.raises(ValueError, match="max_events_per_day must be between 1 and 10"):
    ...
    def test_post_init_validation_invalid_icon_size(self):
        ...
        with pytest.raises(ValueError, match="icon_size must be 'small', 'medium', or 'large'"):
...
class TestAstronomyCacheConfig:
    ...
    def test_post_init_validation_duration_too_low(self):
        ...
        with pytest.raises(ValueError, match="duration_hours must be between 1 and 168"):
    ...
    def test_post_init_validation_cleanup_interval_too_low(self):
        ...
        with pytest.raises(ValueError, match="cleanup_interval_hours must be between 1 and 168"):
...
class TestAstronomyConfig:
    ...
    def test_post_init_validation_empty_api_key_warning(self, mock_logger):
        ...
        mock_logger.warning.assert_called_once_with("Astronomy is enabled but NASA API key is empty")
    ...
    def test_post_init_validation_update_interval_too_low(self):
        ...
        with pytest.raises(ValueError, match="update_interval_minutes must be at least 60"):
    ...
    def test_post_init_validation_timeout_too_low(self):
        ...
        with pytest.raises(ValueError, match="timeout_seconds must be between 5 and 60"):
    ...
    def test_post_init_validation_retry_delay_too_low(self):
        ...
        with pytest.raises(ValueError, match="retry_delay_seconds must be between 1 and 10"):
    ...
    def test_from_file_not_found(self, mock_logger):
        ...
        mock_logger.warning.assert_any_call(f"Astronomy config file not found: {non_existent_path}")
        mock_logger.warning.assert_any_call("Astronomy is enabled but NASA API key is empty")
    ...
    def test_from_file_invalid_json(self, mock_logger):
        ...
        try:
            with pytest.raises(ValueError, match="Invalid astronomy configuration file"):
    ...
    def test_save_to_file_new_file(self, mock_logger):
        ...
        try:
            ...
            mock_logger.info.assert_called_once_with(f"Astronomy configuration saved to {temp_path}")
        finally:
            if temp_path.exists():
                temp_path.unlink()
...
class TestAstronomyConfigManager:
    ...
    def test_init_default_path(self, mock_logger):
        ...
        mock_logger.info.assert_called_once_with("AstronomyConfigManager initialized with path: config.json")
    ...
    def test_init_custom_path(self, mock_logger):
        ...
        assert manager._config is None
        mock_logger.info.assert_called_once_with(f"AstronomyConfigManager initialized with path: {custom_path}")
    ...
    def test_load_config_success(self, mock_logger):
        ...
        try:
            ...
            mock_logger.info.assert_any_call("Astronomy configuration loaded successfully")
        finally:
            temp_path.unlink()
    ...
    def test_save_config_success(self, mock_logger):
        ...
        try:
            manager = AstronomyConfigManager(temp_path)
            config = AstronomyConfig(
                nasa_api_key="save_test",
                location_name="SaveTest"
            )
            
            manager.save_config(config)
            
            assert manager._config == config
            mock_logger.info.assert_called_with("Astronomy configuration saved successfully")
    ...
    @patch('src.managers.astronomy_config.logger')
    @patch.object(AstronomyConfig, 'save_to_file', side_effect=PermissionError("Permission denied"))
    def test_save_config_exception(self, mock_save, mock_logger):
        ...
        mock_logger.error.assert_called_with("Failed to save astronomy configuration: Permission denied")
    ...
    def test_validate_config(self):
        ...
        config = AstronomyConfig(
            enabled=True,
            nasa_api_key=""  # Invalid - missing API key
        )
    ...
    def test_update_api_key(self, mock_logger):
        ...
        try:
            ...
            assert updated_config.location_name == original_location  # Should preserve other values
    ...
    def test_update_location(self, mock_logger):
        ...
        try:
            ...
            assert updated_config.nasa_api_key == original_api_key  # Should preserve other values
    ...
    def test_toggle_service_invalid_service(self):
        ...
        with pytest.raises(ValueError, match="Unknown astronomy service: invalid_service"):
