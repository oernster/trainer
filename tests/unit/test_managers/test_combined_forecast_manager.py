...
@pytest.fixture
def sample_combined_forecast(
    sample_location, sample_weather_data, sample_astronomy_data
):
...
class TestCombinedForecastManagerGetForecast:
    ...
    async def test_get_combined_forecast_weather_only(
        self, mock_weather_manager, sample_weather_data, sample_location
    ):
    ...
    async def test_get_combined_forecast_astronomy_only(
        self, mock_astronomy_manager, sample_astronomy_data, sample_location
    ):
    ...
    async def test_get_combined_forecast_weather_exception(
        ...
        mock_weather_manager.refresh_weather.side_effect = Exception(
            "Weather API error"
        )
        mock_astronomy_manager.refresh_astronomy.return_value = sample_astronomy_data
    ...
    async def test_get_combined_forecast_astronomy_exception(
        ...
        mock_weather_manager.refresh_weather.return_value = sample_weather_data
        mock_astronomy_manager.refresh_astronomy.side_effect = Exception(
            "Astronomy API error"
        )
    ...
    async def test_get_combined_forecast_both_exceptions(
        self, mock_weather_manager, mock_astronomy_manager, sample_location
    ):
    ...
    async def test_get_combined_forecast_general_exception(
        ...
        with patch(
            "src.managers.combined_forecast_manager.CombinedForecastData.create"
        ) as mock_create:
    ...
    async def test_get_combined_forecast_cached(
        ...
        result = await manager.get_combined_forecast(
            sample_location, force_refresh=False
        )
    ...
    async def test_get_combined_forecast_force_refresh(
        ...
        result = await manager.get_combined_forecast(
            sample_location, force_refresh=True
        )
...
class TestCombinedForecastManagerFetchMethods:
    ...
    @pytest.mark.asyncio
    async def test_fetch_weather_data_exception(
        self, mock_weather_manager, sample_location
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_astronomy_data_success(
        self, mock_astronomy_manager, sample_astronomy_data, sample_location
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_astronomy_data_exception(
        self, mock_astronomy_manager, sample_location
    ):
...
class TestCombinedForecastManagerLoadingState:
    ...
    def test_update_combined_loading_state_both_managers(
        self, mock_weather_manager, mock_astronomy_manager
    ):
    ...
    def test_update_combined_loading_state_manager_no_is_loading_method(
        self, mock_weather_manager
    ):
...
class TestCombinedForecastManagerSignalHandlers:
    ...
    async def test_on_weather_updated(self, sample_weather_data):
        ...
        with patch.object(
            manager, "_update_combined_forecast_from_weather"
        ) as mock_update:
    ...
    async def test_on_astronomy_updated(self, sample_astronomy_data):
        ...
        with patch.object(
            manager, "_update_combined_forecast_from_astronomy"
        ) as mock_update:
...
class TestCombinedForecastManagerUpdateMethods:
    ...
    async def test_update_combined_forecast_from_weather_with_existing(
        self, sample_weather_data, sample_combined_forecast
    ):
    ...
    async def test_update_combined_forecast_from_astronomy_no_existing(
        self, sample_astronomy_data
    ):
        """Test updating from astronomy when no existing forecast."""
        manager = CombinedForecastManager()

        with patch.object(manager, "get_combined_forecast") as mock_get:
            await manager._update_combined_forecast_from_astronomy(
                sample_astronomy_data
            )
            mock_get.assert_called_once_with(force_refresh=True)
    ...
    async def test_update_combined_forecast_from_astronomy_with_existing(
        self, sample_astronomy_data, sample_combined_forecast
    ):
...
class TestCombinedForecastManagerGetters:
    ...
    def test_get_cache_info_with_data(
        self, mock_weather_manager, mock_astronomy_manager, sample_combined_forecast
    ):
        ...
        with patch.object(manager._refresh_timer, "start"):
            with patch.object(manager._refresh_timer, "isActive", return_value=True):
                ...
                assert (
                    info["total_astronomy_events"]
                    == sample_combined_forecast.total_astronomy_events
                )
...
class TestCombinedForecastManagerShutdown:
    ...
    def test_shutdown_with_managers(self, mock_weather_manager, mock_astronomy_manager):
...
class TestCombinedForecastFactory:
    ...
    def test_create_manager_both_enabled(self):
        ...
        weather_config.get_refresh_interval_seconds = Mock(
            return_value=300
        )  # Return integer for multiplication
        ...
        with patch(
            "src.managers.combined_forecast_manager.WeatherManager"
        ) as mock_weather_mgr:
            with patch(
                "src.managers.combined_forecast_manager.AstronomyManager"
            ) as mock_astro_mgr:
                ...
                with patch(
                    "src.managers.weather_manager.WeatherManager", mock_weather_mgr
                ):
                    with patch(
                        "src.managers.astronomy_manager.AstronomyManager",
                        mock_astro_mgr,
                    ):
                        result = CombinedForecastFactory.create_manager(
                            weather_config, astronomy_config
                        )
    ...
    def test_create_manager_weather_disabled(self):
        ...
        with patch(
            ...
            with patch(
                "src.managers.astronomy_manager.AstronomyManager", mock_astro_mgr
            ):
                result = CombinedForecastFactory.create_manager(
                    weather_config, astronomy_config
                )
    ...
    def test_create_weather_only_manager(self):
        ...
        with patch(
            ...
            # Also need to patch the import inside the factory method
            with patch("src.managers.weather_manager.WeatherManager", mock_weather_mgr):
                result = CombinedForecastFactory.create_weather_only_manager(
                    weather_config
                )
    ...
    def test_create_astronomy_only_manager(self):
        ...
        with patch(
            ...
            with patch(
                ...
                result = CombinedForecastFactory.create_astronomy_only_manager(
                    astronomy_config
                )
...
class TestCombinedForecastManagerEdgeCases:
    ...
    async def test_get_combined_forecast_validation_failure(
        ...
        with patch.object(
            manager._validator, "validate_combined_forecast", return_value=False
        ):
            # Mock signal emissions
            manager.combined_forecast_updated = Mock()
            manager.combined_forecast_updated.emit = Mock()
