...
class TestRefreshWeatherCommand:
    ...
    async def test_execute(self, mock_weather_manager, mock_location):
        ...
        mock_weather_manager._fetch_weather_internal.assert_called_once_with(
            mock_location
        )
    ...
    async def test_undo_with_previous_data(self, mock_weather_manager, mock_location):
        ...
        mock_weather_manager._set_weather_data.assert_called_once_with(
            mock_previous_data
        )

    @pytest.mark.asyncio
    async def test_undo_without_previous_data(
        self, mock_weather_manager, mock_location
    ):
...
class TestWeatherManager:
    ...
    def test_start_auto_refresh_enabled(self, weather_manager, mock_config):
        ...
        weather_manager._refresh_timer.start.assert_called_once_with(
            1800000
    ...
    @pytest.mark.asyncio
    async def test_fetch_weather_internal_success(
        self, weather_manager, mock_weather_data
    ):
        """Test successful internal weather fetch."""
        weather_manager._api_manager.get_weather_forecast.return_value = (
            mock_weather_data
        )

        with patch.object(
            weather_manager, "notify_loading_state"
        ) as mock_notify_loading:
            with patch.object(
                weather_manager, "notify_weather_updated"
            ) as mock_notify_updated:
    ...
    async def test_fetch_weather_internal_failure(self, weather_manager):
        ...
        with patch.object(
            ...
            with patch.object(
                weather_manager, "notify_weather_error"
            ) as mock_notify_error:
    ...
    def test_update_config_different_interval_active_refresh(
        self, weather_manager, mock_config
    ):
        ...
        with patch("src.managers.weather_manager.WeatherAPIFactory") as mock_factory:
            with patch.object(weather_manager, "stop_auto_refresh") as mock_stop:
                with patch.object(weather_manager, "start_auto_refresh") as mock_start:
                    mock_new_api_manager = AsyncMock()
                    mock_factory.create_manager_from_config.return_value = (
                        mock_new_api_manager
                    )
    ...
    def test_shutdown_with_exception(self, weather_manager):
        ...
        weather_manager._api_manager.shutdown_sync.side_effect = Exception(
            "Shutdown failed"
        )
    ...
    def test_update_config_different_interval_disabled(
        self, weather_manager, mock_config
    ):
    ...
    def test_shutdown_api_manager_shutdown_sync_exception(self):
        ...
        with patch("src.managers.weather_manager.WeatherAPIFactory") as mock_factory:
            mock_api_manager = AsyncMock()
            mock_api_manager.shutdown_sync = Mock(
