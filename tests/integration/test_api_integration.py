...
from src.api.api_manager import APIManager, APIException, NetworkException, AuthenticationException, RateLimitException
...
class TestAPIManagerDepartures:
    ...
    async def test_get_departures_mock_success(self, test_config, test_api_responses):
        ...
        async with APIManager(test_config) as api_manager:
            with patch.object(api_manager.session, 'get', return_value=mock_context_manager) as mock_get:
                ...
                assert params["to_offset"] == f"PT{test_config.display.time_window_hours}H"
    ...
    async def test_get_departures_empty_response(self, test_config, test_api_responses):
        ...
        mock_response = AsyncMock()
        mock_response.status = 200
        mock_response.json = AsyncMock(return_value=test_api_responses["departures_empty"])
    ...
    @pytest.mark.asyncio
    async def test_get_departures_no_data_response(self, test_config, test_api_responses):
        """Test handling of malformed API response."""
        from unittest.mock import patch, AsyncMock
        
        mock_response = AsyncMock()
        mock_response.status = 200
        mock_response.json = AsyncMock(return_value=test_api_responses["departures_no_data"])
    ...
    async def test_get_departures_network_error_with_retry(self, test_config):
        ...
        async with APIManager(test_config) as api_manager:
            with patch.object(api_manager.session, 'get', side_effect=aiohttp.ClientError("Network error")) as mock_get:
...
class TestAPIManagerDataParsing:
    ...
    def test_determine_train_status(self, test_config):
        ...
        assert api_manager._determine_train_status("CANCELLED", 0) == TrainStatus.CANCELLED
        ...
        assert api_manager._determine_train_status("UNKNOWN_STATUS", 0) == TrainStatus.ON_TIME  # Falls through to ON_TIME
    ...
    def test_determine_service_type(self, test_config):
        ...
        assert api_manager._determine_service_type("UNKNOWN") == ServiceType.STOPPING  # Default
        assert api_manager._determine_service_type("") == ServiceType.STOPPING  # Default
...
class TestRealAPIIntegration:
        ...
        if real_app_id and real_app_key:
            ...
            async with APIManager(test_config) as api_manager:
                ...
                assert any(keyword in error_message.lower() for keyword in [
                    "invalid api credentials", "authorisation failed", "authentication",
                    "app_id", "app_key", "application", "not found"
                ])
    ...
    async def test_api_error_handling_with_invalid_credentials(self, test_config):
        ...
        async with APIManager(test_config) as api_manager:
            ...
            error_message = str(exc_info.value)
            assert any(keyword in error_message.lower() for keyword in [
                "invalid api credentials", "authorisation failed", "authentication", "app_id", "app_key"
            ])
    ...
    async def test_10_hour_time_window_parameter(self, test_config):
        ...
        async with APIManager(test_config) as api_manager:
            with patch.object(api_manager.session, 'get', side_effect=capture_params):
                ...
                assert captured_params['to_offset'] == f'PT{test_config.display.time_window_hours}H'